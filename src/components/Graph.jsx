...
export const Graph = ({
  ...
  const fetchStockData = async (startDate, endDate) => {
    try {
      const url = `http://localhost:8000/api/stocks/${symbol}/priceHistory/${startDate}/${endDate}/1d`;
      ...
      let previousClose = -1;
      if (
        (graphPlotType === "candlestick") |
        (graphPlotType === "hollowcandlestick") |
        (graphPlotType === "ohlc")
      ) {
      ...
      } else if (graphPlotType === "heikinashi") {
        chartData = stockData.map((data, index) => {
          if (previousOpen === -1 && previousClose === -1) {
            previousOpen++;
            previousClose++;
            return;
          }
          ...
          const newClose = (open + high + low + close) / 4;
          const newOpen = (previousOpen + previousClose) / 2;
          const newHigh = Math.max(high, newClose, newOpen);
          const newLow = Math.min(low, newClose, newOpen);

          previousOpen = newOpen;
          previousClose = newClose;

          return [timestamp, newOpen, newHigh, newLow, newClose];
        });
      } else if (graphPlotType === "hlc") {
        chartData = stockData.map((data) => {
          const timestamp = new Date(data.Date).getTime();
          timestamps.push(timestamp);
          return [timestamp, data.High, data.Low, data.Close];
        });
      }
  ...
  const updateGraphSeries = () => {
    if (chartRef.current) {
      const chart = chartRef.current;
      if (chart) {
        const series = chart.get("main");
        if (series) {
          series.update({
            type: graphPlotType,
          });
  ...
  }, [stockGraphData, stockVolumeData, graphPlotType]);
