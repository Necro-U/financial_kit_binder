...
const EquitiesChartsPage = () => {
  ...
  // default plot type will be candlestick
  const [graphPlotType, setGraphPlotType] = useState("candlestick");

  const graphPlotTypeOptions = [
    "candlestick",
    "heikinashi",
    "hollowcandlestick",
    "ohlc",
  ...
  const handlePlotTypeClick = (plotType) => {
    setGraphPlotType(plotType);
    handleClosePlotDialog();
  };

  const handleClosePlotDialog = () => {
    ...
    <TabsLayout>
      ...
      >
        ...
        >
          ...
          >
            <h4 style={{ textAlign: "center", width: "100%", margin: "1rem" }}>
              Select Plot Type
            </h4>
            {graphPlotTypeOptions.map((plotType, index) => (
              <Grid
                item
                key={index}
                xs={12}
                onClick={() => handlePlotTypeClick(plotType)}
